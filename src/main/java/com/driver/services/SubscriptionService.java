...
public class SubscriptionService {
    ...
    public Integer upgradeSubscription(Integer userId)throws Exception{
        ...
        Integer previousPlan=subscription.getTotalAmountPaid();
        Integer currentPlan;;
        if(subscription.getSubscriptionType().equals(SubscriptionType.BASIC)){
            subscription.setSubscriptionType(SubscriptionType.PRO);
            currentPlan=previousPlan+300+(50*subscription.getNoOfScreensSubscribed());
        }
        else {
            subscription.setSubscriptionType(SubscriptionType.ELITE);
            currentPlan=previousPlan+200+(100*subscription.getNoOfScreensSubscribed());
        }
        subscription.setTotalAmountPaid(currentPlan);
        user.setSubscription(subscription);
